    ...
    public void generarPreguntaParaBoton(int contador) {
        if (contador < 8) {
            ...
            labelResumen = new Label(
               ...
               try {
                   String mensaje = servicioCorreo.enviar(VaadinSession.getCurrent().getAttribute("nombre")+" "+ VaadinSession.getCurrent().getAttribute("paterno"), labelResumen.getValue());
               }catch(Exception e){
...
try{
                ...
                String  mensajito=  servicioCorreo.enviar(VaadinSession.getCurrent().getAttribute("nombre")+" "+VaadinSession.getCurrent().getAttribute("paterno"), textArea.getValue().toString());
                   textArea.setValue("");
